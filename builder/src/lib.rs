...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Struct(
        ...
        let fields_it = named.iter().map(
            |f| (f.ident.as_ref().expect("Unnamed field"), &f.ty)
        );

        // Builder
        let builder_name: Ident = Ident::new(&format!("{}Builder", name), Span::call_site());

        let builder_fields = fields_it.clone().map(
            |(i, t)| quote! { #i : ::core::option::Option<#t> }
        );
        let builder_setters = fields_it.clone().map(
            |(i, t)| quote! {
